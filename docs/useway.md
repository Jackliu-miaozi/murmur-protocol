...
2. **质押 vDOT 获得 VP**：
   ...
   - **注意**：所有的 VP 都是全局 VP，可以在不同 topic 中使用。VP 不足时只能去 VPToken 合约获得 VP
...
4. **创建议题**：
   ...
   - 参数设置：
     ...
     - `topicDuration_`: `86400` 秒（24 小时）
     - `freezeWindow_`: `600` 秒（10 分钟冻结窗口）
     - `curatedLimit_`: `50`（最多 50 条精选消息）
...
1. **确保有足够的全局 VP**：
   ...
   - 如果 Bob 的全局 VP 不足，需要先调用 `VPToken.stakeVdot()` 质押 vDOT 获得全局 VP
   - 系统计算：`VP = 100 * √vDOT`（例如：1000 vDOT 获得约 3162 VP）
   - **注意**：所有的 VP 都是全局 VP，可以直接在不同 topic 中使用。VP 不足时只能去 VPToken 合约获得 VP
...
2. **发布消息**：
   ...
   - 调用 `MessageRegistry.postMessage(topicId, contentHash, length, aiScore, signature)`：
     ...
     - **限速机制**：
       ...
       - 如果违反限速规则，交易会失败
     - 从用户的全局 VP 中销毁相应数量的 VP
     - 消息 contentHash 存储在链上，获得 `messageId = 1`
     - **错误处理**：
       - 如果全局 VP 余额不足，交易失败，用户需要先去 VPToken 合约获得更多 VP
...
**时间：2024 年 1 月 2 日 00:20**

Bob 想赎回之前质押的 vDOT：

- 调用 `VPToken.withdrawVdot(amount)` 提取质押的 vDOT
- **赎回规则**：
  - 用户可以随时提取已质押的 vDOT（只要不超过已质押的数量）
  - vDOT 的提取不会影响已获得的 VP，但提取后对应的 VP 仍然有效
  - 如果用户想完全退出，需要先提取 vDOT，VP 会继续保留在账户中
- 赎回条件满足后，可以返回相应数量的 vDOT 给 Bob
- **注意**：通过 NFT 铸造返还的 VP 可以继续使用（可用于其他 topic 或创建新 topic）
